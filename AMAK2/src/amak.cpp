...
version_info+="jjm model V1.00.00 July 2022";
 iseed=1313;
  pad_mceval = new ofstream("mceval.rep");
  pad_mclike = new ofstream("mclike.csv");
long int lseed=iseed;
 ...
 mcflag   = 1;
  ...
  {
    oper_mod  = 1;
    nsims = atoi(argv[on+1]);
    cmp_no = atoi(argv[on+1]);
    cout<<"Got to operating model option "<<oper_mod<<" CMP No: "<<cmp_no<<" Nsims="<<nsims<<endl;
  }
 ...
 *(ad_comm::global_datafile) >>  model_name; 
 ...
 log_input(wt_fsh);
  nind.allocate("nind");
  mina.allocate(1,nind);
 log_input(nind);
 nfsh_and_ind = nfsh+nind;
  Francis_wts.allocate(1,nfsh_and_ind);
  ...
  *(ad_comm::global_datafile) >>  model_name; 
  ...
  nstk.allocate("nstk");
  hcr_tac.allocate(1,nstk);
 hcr_tac_in = 10 ;
  pstkname.allocate(1,nstk,1,2);
  stknameread.allocate("stknameread");
  ...
  sel_map.allocate(1,3,1,nfsh_and_ind,"sel_map");
 log_input(datafile_name);
 log_input(model_name);
 log_input(nstk);
 log_input(stkname);
 ...
 projfile_name = cntrlfile_name(1,length(cntrlfile_name)-4) + ".prj";
  nreg.allocate(1,nstk,"nreg");
 log_input(nreg);
 nregs = sum(nreg);
  cum_regs.allocate(1,nstk);
 cum_regs.initialize();
  R_guess.allocate(1,nregs);
  stk_reg_map.allocate(1,2,1,nregs);
  //cum_regs(1) = 0;
  for (s=2;s<=nstk;s++)
  {
 ...
 log_input(retro);
  rec_map.allocate(1,nstk,1,nreg,"rec_map");
  phase_mean_rec.allocate(1,nregs);
  nrec = max(rec_map);
  phase_mean_rec = 1;
  ...
  steepnessprior.allocate(1,nrec,"steepnessprior");
  cvsteepnessprior.allocate(1,nrec,"cvsteepnessprior");
  phase_srec.allocate(1,nrec,"phase_srec");
  sigmarprior.allocate(1,nrec,"sigmarprior");
  log_sigmarprior.allocate(1,nrec);
  cvsigmarprior.allocate(1,nrec,"cvsigmarprior");
  phase_sigmar.allocate(1,nrec,"phase_sigmar");
 log_input(sigmarprior);
 log_input(cvsigmarprior);
 log_input(phase_sigmar);
  phase_Rzero.allocate(1,nregs,"phase_Rzero");
  nrecs_est_shift.allocate(1,nregs,"nrecs_est_shift");
  yr_rec_est.allocate(1,nregs,1,nrecs_est_shift,"yr_rec_est");
  styr_rec_est.allocate(1,nstk,1,nreg);
  endyr_rec_est.allocate(1,nstk,1,nreg);
  for (s=1;s<=nstk;s++)
  {
    for (r=1;r<=nreg(s);r++)
    {
 ...
 log_input(yr_rec_est);
  reg_shift.allocate(1,nstk,1,nreg-1,"reg_shift");
  yy_shift_st.allocate(1,nstk,1,nreg);
  yy_shift_end.allocate(1,nstk,1,nreg);
  growth_map.allocate(1,nstk,1,nreg,"growth_map");
  ngrowth = max(growth_map);
  Linfprior.allocate(1,ngrowth,"Linfprior");
  cvLinfprior.allocate(1,ngrowth,"cvLinfprior");
  phase_Linf.allocate(1,ngrowth,"phase_Linf");
 ...
 lw_b = 3.089248476 ; // LW parameters from Talcahuano sampling, 2008
  log_Linfprior.allocate(1,ngrowth);
 log_Linfprior = log(Linfprior);
 log_input(Linfprior)
 log_input(cvLinfprior)
  kprior.allocate(1,ngrowth,"kprior");
  cvkprior.allocate(1,ngrowth,"cvkprior");
  phase_k.allocate(1,ngrowth,"phase_k");
  log_kprior.allocate(1,ngrowth);
 log_kprior = log(kprior);
 log_input(kprior)
 log_input(cvkprior)
  Loprior.allocate(1,ngrowth,"Loprior");
  cvLoprior.allocate(1,ngrowth,"cvLoprior");
  phase_Lo.allocate(1,ngrowth,"phase_Lo");
  log_Loprior.allocate(1,ngrowth);
 log_Loprior = log(Loprior);
 log_input(Loprior)
 log_input(cvLoprior)
  sdageprior.allocate(1,ngrowth,"sdageprior");
  cvsdageprior.allocate(1,ngrowth,"cvsdageprior");
  phase_sdage.allocate(1,ngrowth,"phase_sdage");
  log_sdageprior.allocate(1,ngrowth);
 log_sdageprior = log(sdageprior);
 log_input(sdageprior)
 log_input(cvsdageprior)
  mort_map.allocate(1,nstk,1,nreg,"mort_map");
  nmort = max(mort_map);
  npar = max(max(nrec,ngrowth),nmort);
  natmortprior.allocate(1,nmort,"natmortprior");
  cvnatmortprior.allocate(1,nmort,"cvnatmortprior");
  phase_M.allocate(1,nmort,"phase_M");
 log_input(natmortprior);
 log_input(cvnatmortprior);
 log_input(phase_M);
  npars_Mage.allocate(1,nmort,"npars_Mage");
  ages_M_changes.allocate(1,nmort,1,npars_Mage,"ages_M_changes");
  Mage_in.allocate(1,nmort,1,npars_Mage,"Mage_in");
  phase_Mage.allocate(1,nmort,"phase_Mage");
  Mage_offset_in.allocate(1,nmort,1,npars_Mage);
 ...
 log_input(phase_Mage);
  phase_rw_M.allocate(1,nstk,"phase_rw_M");
  npars_rw_M.allocate(1,nstk,"npars_rw_M");
  yrs_rw_M.allocate(1,nstk,1,npars_rw_M,"yrs_rw_M");
  sigma_rw_M.allocate(1,nstk,1,npars_rw_M,"sigma_rw_M");
 ...
 log_input(q_age_min);
 log_input(q_age_max);
  use_vb_wt_age.allocate("use_vb_wt_age");
  log_input(use_vb_wt_age);
cv_catchbiomass = 0.05; // 1./(2*0.05*0.05);
catchbiomass_pen= 200.; // 1./(2*0.05*0.05);
  nproj_yrs.allocate("nproj_yrs");
  log_input(nproj_yrs);
 phase_nosr  = -3;
  yy_sr.allocate(1,nstk,styr_sp,endyr+nproj_yrs);
  yy_sr = 1;
  for (s=1;s<=nstk;s++)
  {
    for (r=2;r<=nreg(s);r++)
  ...
  logsel_p1_in_fsh.allocate(1,nfsh,1,nyrs);
  logsel_p1_in_fshv.allocate(1,nfsh);
  sel_p1_in_fsh.allocate(1,nfsh,1,nyrs);
  logsel_p2_in_fsh.allocate(1,nfsh,1,nyrs);
  logsel_p2_in_fshv.allocate(1,nfsh);
  sel_p2_in_fsh.allocate(1,nfsh,1,nyrs);
  logsel_p3_in_fsh.allocate(1,nfsh,1,nyrs);
  logsel_p3_in_fshv.allocate(1,nfsh);
  sel_p3_in_fsh.allocate(1,nfsh,1,nyrs);
 ...
 yrs_sel_ch_tmp_ind.initialize();
  ...
  logsel_p1_in_ind.allocate(1,nind,1,nyrs);
  logsel_p1_in_indv.allocate(1,nind);
  sel_p1_in_ind.allocate(1,nind,1,nyrs);
  sel_p2_in_ind.allocate(1,nind,1,nyrs);
  sel_p2_in_indv.allocate(1,nind);
  logsel_p3_in_ind.allocate(1,nind,1,nyrs);
  logsel_p3_in_indv.allocate(1,nind);
  sel_p3_in_ind.allocate(1,nind,1,nyrs);
 ...
 sel_sigma_ind.initialize();
    ...
    switch (fsh_sel_opt(k))
    {
      ...
      case 3 : // Double logistic 
      {
        ...
        *(ad_comm::global_datafile) >> sel_p3_in_fsh(k,1) ; // ages to desc infl
        ...
        log_input(phase_sel_fsh(k));
        log_input(n_sel_ch_fsh(k));
        log_input(sel_sigma_fsh(k)(1,n_sel_ch_fsh(k)));
  ...
  wt_pop.allocate(1,nstk,1,nages,"wt_pop");
 log_input(wt_pop);
  maturity.allocate(1,nstk,1,nages,"maturity");
 log_input(maturity);
  wt_mature.allocate(1,nstk,1,nages);
 ...
 write_input_log<<"# Curv_pen_ind: "<<endl<<curv_pen_ind<<endl;
  ...
  fsh_zero_catch.allocate(1,200);
  yrs_zero_catch.allocate(1,200);
 ...
 do_fmort=0;
  ...
  for (k=1;k<=nind;k++)
  {
    ...
    {
      oac_ind(k,i)(mina(k),nages) /= sum(oac_ind(k,i)(mina(k),nages)); // Normalize to sum to one
      offset_ind(k) -= n_sample_ind_age(k,i)*(oac_ind(k,i)(mina(k),nages) + 0.001) * log(oac_ind(k,i)(mina(k),nages) + 0.001 ) ;
    }
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  Mest.allocate(1,nmort,.02,4.8,phase_M,"Mest");
  Mage_offset.allocate(1,nmort,1,npars_Mage,-3,3,phase_Mage,"Mage_offset");
  Mage.allocate(1,nmort,1,nages,"Mage");
  #ifndef NO_AD_INITIALIZE
    Mage.initialize();
  #endif
  M_rw.allocate(1,nstk,1,npars_rw_M,-10,10,phase_rw_M,"M_rw");
  natmort.allocate(1,nstk,styr,endyr,"natmort");
  #ifndef NO_AD_INITIALIZE
    natmort.initialize();
  #endif
  natage.allocate(1,nstk,styr,endyr+1,1,nages,"natage");
  #ifndef NO_AD_INITIALIZE
    natage.initialize();
  #endif
  N_NoFsh.allocate(1,nstk,styr,endyr_fut,1,nages,"N_NoFsh");
  #ifndef NO_AD_INITIALIZE
    N_NoFsh.initialize();
  #endif
  pred_rec.allocate(1,nstk,styr_rec,endyr,"pred_rec");
  #ifndef NO_AD_INITIALIZE
    pred_rec.initialize();
  #endif
  mod_rec.allocate(1,nstk,styr_rec,endyr,"mod_rec");
  #ifndef NO_AD_INITIALIZE
    mod_rec.initialize();
  #endif
  M.allocate(1,nstk,styr,endyr,1,nages,"M");
  #ifndef NO_AD_INITIALIZE
    M.initialize();
  #endif
  Z.allocate(1,nstk,styr,endyr,1,nages,"Z");
  #ifndef NO_AD_INITIALIZE
    Z.initialize();
  #endif
  S.allocate(1,nstk,styr,endyr,1,nages,"S");
  #ifndef NO_AD_INITIALIZE
    S.initialize();
  #endif
  log_Linf.allocate(1,ngrowth,phase_Linf,"log_Linf");
  log_k.allocate(1,ngrowth,phase_k,"log_k");
  log_Lo.allocate(1,ngrowth,phase_Lo,"log_Lo");
  log_sdage.allocate(1,ngrowth,phase_sdage,"log_sdage");
  wt_age_vb.allocate(1,ngrowth,1,nages,"wt_age_vb");
  #ifndef NO_AD_INITIALIZE
    wt_age_vb.initialize();
  #endif
  maturity_vb.allocate(1,ngrowth,1,nages,"maturity_vb");
  #ifndef NO_AD_INITIALIZE
    maturity_vb.initialize();
  #endif
  mean_log_rec.allocate(1,nregs,phase_mean_rec,"mean_log_rec");
  steepness.allocate(1,nrec,0.21,Steepness_UB,phase_srec,"steepness");
  log_Rzero.allocate(1,nregs,phase_Rzero,"log_Rzero");
  rec_dev.allocate(1,nstk,styr_rec,endyr,-15,15,2,"rec_dev");
  log_sigmar.allocate(1,nrec,phase_sigmar,"log_sigmar");
  m_sigmarsq.allocate(1,nregs,"m_sigmarsq");
  #ifndef NO_AD_INITIALIZE
    m_sigmarsq.initialize();
  #endif
  m_sigmar.allocate(1,nregs,"m_sigmar");
  #ifndef NO_AD_INITIALIZE
    m_sigmar.initialize();
  #endif
  sigmarsq.allocate(1,nregs,"sigmarsq");
  #ifndef NO_AD_INITIALIZE
    sigmarsq.initialize();
  #endif
  sigmar.allocate(1,nrec,"sigmar");
  #ifndef NO_AD_INITIALIZE
    sigmar.initialize();
  #endif
  alpha.allocate(1,nregs,"alpha");
  #ifndef NO_AD_INITIALIZE
    alpha.initialize();
  #endif
  beta.allocate(1,nregs,"beta");
  #ifndef NO_AD_INITIALIZE
    beta.initialize();
  #endif
  Bzero.allocate(1,nregs,"Bzero");
  #ifndef NO_AD_INITIALIZE
    Bzero.initialize();
  #endif
  Rzero.allocate(1,nregs,"Rzero");
  #ifndef NO_AD_INITIALIZE
    Rzero.initialize();
  #endif
  phizero.allocate(1,nregs,"phizero");
  #ifndef NO_AD_INITIALIZE
    phizero.initialize();
  #endif
  avg_rec_dev.allocate(1,nregs,"avg_rec_dev");
  #ifndef NO_AD_INITIALIZE
    avg_rec_dev.initialize();
  #endif
  fmort.allocate(1,nfsh,styr,endyr,-15,15.,phase_fmort,"fmort");
  Fmort.allocate(1,nstk,styr,endyr,"Fmort");
  #ifndef NO_AD_INITIALIZE
    Fmort.initialize();
  #endif
 ...
 log_input(phase_selcoff_fsh);
  ...
  logsel_p1_fsh.allocate(1,nfsh,1,n_sel_ch_fsh,phase_dlogist_fsh,"logsel_p1_fsh");
  sel_p1_fsh.allocate(1,nfsh,1,n_sel_ch_fsh,"sel_p1_fsh");
  #ifndef NO_AD_INITIALIZE
    sel_p1_fsh.initialize();
  #endif
  logsel_p2_fsh.allocate(1,nfsh,1,n_sel_ch_fsh,phase_dlogist_fsh,"logsel_p2_fsh");
  sel_p2_fsh.allocate(1,nfsh,1,n_sel_ch_fsh,"sel_p2_fsh");
  #ifndef NO_AD_INITIALIZE
    sel_p2_fsh.initialize();
  #endif
  logsel_p3_fsh.allocate(1,nfsh,1,n_sel_ch_fsh,-10,10,phase_dlogist_fsh,"logsel_p3_fsh");
  sel_p3_fsh.allocate(1,nfsh,1,n_sel_ch_fsh,"sel_p3_fsh");
  #ifndef NO_AD_INITIALIZE
    sel_p3_fsh.initialize();
  #endif
 int lb_d50=nages/2;
  ...
  #endif
  Ftot.allocate(1,nstk,styr,endyr,1,nages,"Ftot");
  #ifndef NO_AD_INITIALIZE
    Ftot.initialize();
  #endif
  ...
  catage.allocate(1,nfsh,styr,endyr,1,nages,"catage");
    ...
    catage.initialize();
  #endif
  catage_tot.allocate(1,nstk,styr,endyr,1,nages,"catage_tot");
  #ifndef NO_AD_INITIALIZE
  ...
  F35.allocate(1,nfsh,"F35");
    ...
    F35.initialize();
  #endif
  sigmar_fut.allocate(1,nstk,"sigmar_fut");
  #ifndef NO_AD_INITIALIZE
 ...
 Fratio /= sum(Fratio);
  Nspr.allocate(1,4,1,nages,"Nspr");
    ...
    Nspr.initialize();
  #endif
  nage_future.allocate(1,nstk,styr_fut,endyr_fut,1,nages,"nage_future");
  #ifndef NO_AD_INITIALIZE
    nage_future.initialize();
  #endif
  rec_dev_future.allocate(1,nstk,styr_fut,endyr_fut,phase_proj,"rec_dev_future");
  Sp_Biom_future.allocate(1,nstk,styr_fut-rec_age,endyr_fut,"Sp_Biom_future");
  #ifndef NO_AD_INITIALIZE
  ...
  F_future.allocate(1,nfsh,styr_fut,endyr_fut,1,nages,"F_future");
    ...
    F_future.initialize();
  #endif
  Z_future.allocate(1,nstk,styr_fut,endyr_fut,1,nages,"Z_future");
  #ifndef NO_AD_INITIALIZE
    Z_future.initialize();
  #endif
  S_future.allocate(1,nstk,styr_fut,endyr_fut,1,nages,"S_future");
  #ifndef NO_AD_INITIALIZE
    S_future.initialize();
  #endif
  catage_future.allocate(1,nstk,styr_fut,endyr_fut,1,nages,"catage_future");
  #ifndef NO_AD_INITIALIZE
    catage_future.initialize();
  #endif
  avg_rec_dev_future.allocate(1,nstk,"avg_rec_dev_future");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  sel50_ind.allocate(1,nind,1,n_sel_ch_ind,0,nages,phase_logist_ind,"sel50_ind");
  logsel_p1_ind.allocate(1,nind,1,n_sel_ch_ind,phase_dlogist_ind,"logsel_p1_ind");
  sel_p1_ind.allocate(1,nind,1,n_sel_ch_ind,"sel_p1_ind");
  #ifndef NO_AD_INITIALIZE
    sel_p1_ind.initialize();
  #endif
  sel_p2_ind.allocate(1,nind,1,n_sel_ch_ind,phase_dlogist_ind,"sel_p2_ind");
  logsel_p3_ind.allocate(1,nind,1,n_sel_ch_ind,phase_dlogist_ind,"logsel_p3_ind");
  sel_p3_ind.allocate(1,nind,1,n_sel_ch_ind,"sel_p3_ind");
  #ifndef NO_AD_INITIALIZE
    sel_p3_ind.initialize();
  #endif
  ...
  sigma.initialize();
  #endif
  rec_like.allocate(1,nstk,1,4,"rec_like");
  #ifndef NO_AD_INITIALIZE
    rec_like.initialize();
  #endif
  ...
  fpen.allocate(1,6,"fpen");
    ...
    fpen.initialize();
  #endif
  post_priors.allocate(1,npar,1,8,"post_priors");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  repl_F.allocate(1,nstk,"repl_F");
  #ifndef NO_AD_INITIALIZE
    repl_F.initialize();
  #endif
  repl_yld.allocate(1,nstk,"repl_yld");
  #ifndef NO_AD_INITIALIZE
    repl_yld.initialize();
  #endif
  repl_SSB.allocate(1,nstk,"repl_SSB");
  #ifndef NO_AD_INITIALIZE
    repl_SSB.initialize();
  #endif
  B100.allocate(1,nstk,"B100");
  F50_est.allocate(1,nstk,"F50_est");
  #ifndef NO_AD_INITIALIZE
    F50_est.initialize();
  #endif
  F40_est.allocate(1,nstk,"F40_est");
  #ifndef NO_AD_INITIALIZE
    F40_est.initialize();
  #endif
  F35_est.allocate(1,nstk,"F35_est");
  #ifndef NO_AD_INITIALIZE
    F35_est.initialize();
  #endif
  ...
  #endif
  totbiom.allocate(1,nstk,styr,endyr+1,"totbiom");
  totbiom_NoFish.allocate(1,nstk,styr,endyr,"totbiom_NoFish");
  Sp_Biom.allocate(1,nstk,styr_sp,endyr+1,"Sp_Biom");
  Sp_Biom_NoFish.allocate(1,nstk,styr_sp,endyr_fut,"Sp_Biom_NoFish");
  Sp_Biom_NoFishRatio.allocate(1,nstk,styr+1,endyr,"Sp_Biom_NoFishRatio");
  ABCBiom.allocate(1,nstk,"ABCBiom");
  recruits.allocate(1,nstk,styr,endyr+1,"recruits");
  depletion.allocate(1,nstk,"depletion");
  depletion_dyn.allocate(1,nstk,"depletion_dyn");
  MSY.allocate(1,nstk,"MSY");
  MSYL.allocate(1,nstk,"MSYL");
  Fmsy.allocate(1,nstk,"Fmsy");
  lnFmsy.allocate(1,nstk,"lnFmsy");
  Fcur_Fmsy.allocate(1,nstk,"Fcur_Fmsy");
  Rmsy.allocate(1,nstk,"Rmsy");
  Bmsy.allocate(1,nstk,"Bmsy");
  Bcur_Bmsy.allocate(1,nstk,"Bcur_Bmsy");
  pred_ind_nextyr.allocate(1,nind,"pred_ind_nextyr");
  OFL.allocate(1,nstk,"OFL");
  catch_future.allocate(1,nstk,1,5,styr_fut,endyr_fut,"catch_future");
  #ifndef NO_AD_INITIALIZE
    catch_future.initialize();
  #endif
  SSB_fut.allocate(1,nstk,1,6,styr_fut,endyr_fut,"SSB_fut");
  #ifndef NO_AD_INITIALIZE
    SSB_fut.initialize();
  #endif
  SSB_fut_1.allocate(1,nstk,styr_fut,endyr_fut,"SSB_fut_1");
  SSB_fut_2.allocate(1,nstk,styr_fut,endyr_fut,"SSB_fut_2");
  SSB_fut_3.allocate(1,nstk,styr_fut,endyr_fut,"SSB_fut_3");
  SSB_fut_4.allocate(1,nstk,styr_fut,endyr_fut,"SSB_fut_4");
  SSB_fut_5.allocate(1,nstk,styr_fut,endyr_fut,"SSB_fut_5");
  SSB_fut_6.allocate(1,nstk,styr_fut,endyr_fut,"SSB_fut_6");
 ...
 write_input_log<< "exp(log_sigmarprior "<<exp(log_sigmarprior)<<endl;
  Linf.allocate(1,ngrowth,"Linf");
  #ifndef NO_AD_INITIALIZE
    Linf.initialize();
  #endif
  k_coeff.allocate(1,ngrowth,"k_coeff");
  #ifndef NO_AD_INITIALIZE
    k_coeff.initialize();
  #endif
  Lo.allocate(1,ngrowth,"Lo");
  #ifndef NO_AD_INITIALIZE
    Lo.initialize();
  #endif
  sdage.allocate(1,ngrowth,"sdage");
  #ifndef NO_AD_INITIALIZE
    sdage.initialize();
  #endif
  mu_age.allocate(1,ngrowth,1,nages,"mu_age");
  #ifndef NO_AD_INITIALIZE
    mu_age.initialize();
  #endif
  sigma_age.allocate(1,ngrowth,1,nages,"sigma_age");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  P_age2len.allocate(1,ngrowth,1,nages,1,nlength,"P_age2len");
  #ifndef NO_AD_INITIALIZE
...
void model_parameters::preliminary_calculations(void)
{
  ...
  log_input(Mage_offset_in);
  write_input_log << "# Mage_offset " << endl<< Mage_offset(1) << endl;
  log_input(M);
  Get_Age2length();
...
void model_parameters::initializationfunction(void)
{
  ...
  logsel_p1_fsh.set_initial_value(logsel_p1_in_fshv);
  logsel_p2_fsh.set_initial_value(logsel_p2_in_fshv);
  logsel_p3_fsh.set_initial_value(logsel_p3_in_fshv);
  logsel_p1_ind.set_initial_value(logsel_p1_in_indv);
  sel_p2_ind.set_initial_value(sel_p2_in_indv);
  logsel_p3_ind.set_initial_value(logsel_p3_in_indv);
...
void model_parameters::write_mceval(void)
{
  ...
  */
  ...
  for (int k=1;k<=nstk;k++)
  {
    for (i=styr;i<=endyr;i++)
    {
      mceval<< mc_count<<" SSB      "<<i<<" all_stock_"<<k<<" "<<Sp_Biom(k,i)<<  endl;
      mceval<< mc_count<<" Recruits "<<i<<" Age_"<<rec_age<<"_stock_"<<k<<" "<<recruits(k,i)<<  endl;
      // dvector seltmp2(1,nages); seltmp2.initialize(); // for (k=1;k<=nfsh;k++) seltmp2 += F(k,i)   ;       seltmp2 /= mean(seltmp2);
      // This line for looking at mceval for selectivity... for (j=1;j<=nages;j++) { mceval<< mc_count<<" sel "<<i<<" "<<j<<" "<<sel_fsh(2,i,j)<<  endl; // F(k,i)   =  fmort(k,i) * sel_fsh(k,i) ; }
      // 3darray catage(1,nfsh,styr,endyr,1,nages)
      // This line for looking at mceval for selectivity... 
      for (j=1;j<=nages;j++) 
      { 
        mceval<< mc_count<<" C_stock_"<<k<<" "<<i<<" "<<j<<" "<<catage(k,i,j)<<  endl; 
      }
    }
...
dvariable model_parameters::repl_ssb(dvariable& Ftrial,const int& istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
void model_parameters::Get_Survey_Predictions(void)
{
  ...
  for (k=1;k<=nind;k++)
  {
    ...
    natagetmp(1)       = SRecruit(Sp_Biom(istk,endyr+1-rec_age),cum_regs(istk)+yy_sr(istk,styr_fut));
...
void model_parameters::Catch_at_Age(const int& s, const int& i)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
void model_parameters::get_future_Fs(const int& s,const int& i,const int& iscenario)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvar_vector model_parameters::yld(const dvar_vector& Fratio, const dvariable& Ftmp,int istk,int iyr)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvar_vector model_parameters::yld(const dvar_vector& Fratio, const dvariable& Ftmp, int istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::yield(const dvar_vector& Fratio, const dvariable& Ftmp,int istk,int iyr)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::yield(const dvar_vector& Fratio, const dvariable& Ftmp,int istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::yield(const dvar_vector& Fratio, dvariable& Ftmp, dvariable& Stmp,dvariable& Req, int istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvar_vector model_parameters::SRecruit(const dvar_vector& Stmp, const int& Nsr_tmp)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::SRecruit(const double& Stmp, const int& Nsr_tmp)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::SRecruit(const dvariable& Stmp,const int& Nsr_tmp)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::Requil(dvariable& phi, int iyr, int istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
 ...
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  cout <<"_______________________________________________________________"<<endl;
    ...
    for (k=1;k<=nind;k++)
    {
      ...
      for (i=1;i<=nyrs_ind_age(k);i++) 
        report << yrs_ind_age(k,i)<< " "<< oac_ind(k,i)(mina(k),nages) << endl;
    }
    for (k=1;k<=nind;k++)
    {
...
void model_parameters::set_runtime(void)
{
  ...
  dvector temp1("{100,200,300,1500,25000}");
...
dvariable model_parameters::get_spr_rates(double spr_percent,int istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::spr_ratio(dvariable trial_F,dvar_matrix sel_tmp,int iyr,int istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::spr_unfished(int istk,int i)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::SolveF3(const int& iyr, const dvar_vector& N_tmp, const double&  TACin, const int& istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvariable model_parameters::SolveF2(const int& iyr, const dvar_vector& N_tmp, const double&  TACin, const int& istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
dvar_vector model_parameters::SolveF2(const int& iyr, const dvector&  Catch, const int& istk)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
    ...
    for (k=1;k<=nind;k++)
    {
      if (sel_map(1,k+nfsh) == s)
      {
        ...
        { 
          ...
          for (i=1;i<=nyrs_ind_age(k);i++) 
            R_report << yrs_ind_age(k,i)<< " "<< oac_ind(k,i)(mina(k),nages) << endl;
          R_report   << endl;
          ...
          for (i=1;i<=nyrs_ind_age(k);i++) 
            R_report << yrs_ind_age(k,i)<< " "<< sdnr( eac_ind(k,i)(mina(k),nages),oac_ind(k,i)(mina(k),nages),n_sample_ind_age(k,i)) << endl;
          R_report   << endl;
        }
        if (nyrs_ind_length(k)>0) 
        { 
    ...
    for (k=1;k<=nind;k++)
    {
      if (sel_map(1,k+nfsh) == s)
      {
        if (nyrs_ind_age(k)>0)
        {
          ...
          for (i=1;i<=nyrs_ind_age(k);i++)
          {
            double sda_tmp = Sd_age(oac_ind(k,i)(mina(k),nages),age_vector(mina(k),nages));
            R_report << yrs_ind_age(k,i)
                     << " "<<Eff_N(oac_ind(k,i)(mina(k),nages),eac_ind(k,i)(mina(k),nages)) 
                     << " "<<Eff_N2(oac_ind(k,i)(mina(k),nages),eac_ind(k,i)(mina(k),nages))
                     << " "<<mn_age(oac_ind(k,i)(mina(k),nages),age_vector(mina(k),nages))
                     << " "<<mn_age(eac_ind(k,i)(mina(k),nages),age_vector(mina(k),nages))
                     << " "<<sda_tmp
                     << " "<<mn_age(oac_ind(k,i)(mina(k),nages),age_vector(mina(k),nages)) - sda_tmp *2. / sqrt(n_sample_ind_age(k,i))
                     << " "<<mn_age(oac_ind(k,i)(mina(k),nages),age_vector(mina(k),nages)) + sda_tmp *2. / sqrt(n_sample_ind_age(k,i))
                     <<endl;
          }
        }
...
double model_parameters::mn_age(const dvar_vector& pobs,const dvector& age_vector)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
double model_parameters::mn_age(const dvector& pobs,const dvector& age_vector)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
double model_parameters::Sd_age(const dvector& pobs,const dvector& age_vector)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
...
double model_parameters::calc_Francis_weights(dmatrix oac, dvar_matrix eac, ivector sam,const int mina )
{
  ofstream& mceval= *pad_mceval;
  ofstream& mclike= *pad_mclike;
  random_number_generator& rng= *pad_rng;
  {
    ...
    int i2=oac.rowmax();
    double lfwt,Var,Pre,Obs;
    dvector ages(mina,nages);
    for (int i=mina;i<=nages;i++) 
      ages(i) = double(i);
    nobs = oac.rowsize();
    dvector resid(i1,i2);
    resid.initialize();
    ...
    {
      // Obs = sum(elem_prod(oac(i), ages+.5));
      Obs = oac(i)(mina,nages) * (ages+.5);
      // Pre = sum(elem_prod(value(eac(i)), ages+.5));
      Pre = value(eac(i)(mina,nages)) * (ages+.5);
      Var = value(eac(i)(mina,nages)) * square(ages+.5);
      Var -= square(Pre);
      resid(i) = (Obs - Pre) / sqrt(Var * 1.0 / (sam(i) ));
    }
    lfwt = 1.0 / (square(std_dev(resid)) * ((nobs - 1.0) / nobs * 1.0));
