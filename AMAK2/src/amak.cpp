...
catchbiomass_pen= 1./(2*cv_catchbiomass*cv_catchbiomass);
 ...
 yrs_sel_ch_tmp_ind.initialize();
  ...
  logsel_p2_in_ind.allocate(1,nind,1,nyrs);
  logsel_p2_in_indv.allocate(1,nind);
  sel_p2_in_ind.allocate(1,nfsh,1,nyrs);
    ...
    switch (ind_sel_opt(k))
    {
      ...
      case 3 : // Double logistic 
      {
        ...
        *(ad_comm::global_datafile) >> sel_p3_in_ind(k,1) ; // ages to desc infl
        ...
        logsel_p2_in_indv(k) = logsel_p2_in_ind(k,1);
 ...
 write_input_log<<"# Curv_pen_ind: "<<endl<<curv_pen_ind<<endl;
  ...
  // obs_se_ind_in(1,nind,1,nyrs_ind)       //values of indices serrs
    if(surv_dwnwt==1){
			if(yrs_ind_in(k,nyrs_ind(k))  <= endyr) 
        obs_se_ind_in(k,nyrs_ind(k)) *= cv_inc;      //values of indices serrs
			if(yrs_ind_in(k,nyrs_ind(k))  <=  endyr) 
        n_sample_ind_age_in(k,nyrs_ind_age(k)) = n_comp;
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  #endif
  fmort.allocate(1,nfsh,styr,endyr,0.00,15.,phase_fmort,"fmort");
 ...
 log_input(phase_selcoff_fsh);
  ...
  #endif
 if (phase_dlogist_fsh[1]>0) for (k=1;k<=nfsh;k++) if (phase_dlogist_fsh[k]>0) phase_dlogist_fsh[k]++;
  ...
  #endif
 if (phase_dlogist_fsh[1]>0) for (k=1;k<=nfsh;k++) if (phase_dlogist_fsh[k]>0) phase_dlogist_fsh[k]++;
  logsel_p3_fsh.allocate(1,nfsh,1,n_sel_ch_fsh,phase_dlogist_fsh,"logsel_p3_fsh");
 ...
 Fratio /= sum(Fratio);
  ...
  #endif
 if (phase_dlogist_ind[1]>0) for (k=1;k<=nind;k++) if (phase_dlogist_ind[k]>0) phase_dlogist_ind[k]++;
  logsel_p2_ind.allocate(1,nind,1,n_sel_ch_ind,phase_dlogist_ind,"logsel_p2_ind");
  sel_p2_ind.allocate(1,nind,1,n_sel_ch_ind,"sel_p2_ind");
  #ifndef NO_AD_INITIALIZE
    sel_p2_ind.initialize();
  #endif
 if (phase_dlogist_ind[1]>0) for (k=1;k<=nind;k++) if (phase_dlogist_ind[k]>0) phase_dlogist_ind[k]++;
  ...
  Sp_Biom_NoFish.allocate(styr_sp,endyr,"Sp_Biom_NoFish");
  Sp_Biom_NoFishRatio.allocate(styr+1,endyr,"Sp_Biom_NoFishRatio");
  ABCBiom.allocate("ABCBiom");
  recruits.allocate(styr_rec,endyr+1,"recruits");
...
void model_parameters::initializationfunction(void)
{
  ...
  logsel_p2_ind.set_initial_value(logsel_p2_in_indv);
...
void model_parameters::write_mceval(void)
{
  ...
  mc_count++;
    ...
    {
      // iyr = int(yrs_ind(k,i));
      mceval_srv<<yrs_ind(k,i)<<" "
			          <<obs_ind(k,i)<<" "
								<< pred_ind(k,i) <<" "
								<< mfexp(rnorm(log(value(pred_ind(k,i))) , obs_lse_ind(k,i), rng)) <<" "
								<< mc_count<<" "
								<< square(exp(obs_lse_ind(k,i))) <<" "
								<<endl;
      // ind_like(k) += square(log(obs_ind(k,i)) - log(pred_ind(k,i)) ) / (2.*obs_lse_ind(k,i)*obs_lse_ind(k,i));
		}
    for (k=1;k<=5;k++)
  ...
  q_ind(1,1)  << " "<< 
  M(endyr,1)    << " "<< 
  sel_p1_fsh(1,n_sel_ch_fsh(1)) <<" "<<         
  sel_p2_fsh(1,n_sel_ch_fsh(1)) <<" "<<         
  sel_p3_fsh(1,n_sel_ch_fsh(1)) <<" "<<         
  steepness << " "<< 
  depletion << " "<< 
  MSY       << " "<< 
  MSYL      << " "<< 
      ...
      case 3 : // Double logistic
        {
          ...
          int isel_ch_tmp = 1 ;
          dvariable bu = sel_p1_ind(k,isel_ch_tmp);
          dvariable du = sel_p2_ind(k,isel_ch_tmp);
          dvariable dd = sel_p3_ind(k,isel_ch_tmp);
			    dvariable bd = bu + du + dd;
          if (isel_ch_tmp<n_sel_ch_ind(k))
            isel_ch_tmp++;
        log_sel_ind(k,i)(1,nselages_ind(k))     = ( -log(1.0 + mfexp(-2.9444389792/du * ( age_vector(1,nselages_ind(k)) - bu) )) +
               log(1. - 1./(1.0 + mfexp(-2.9444389792/dd * ( age_vector(1,nselages_ind(k)) - bd))) ) )+0.102586589 ; // constant at end is log(0.95*0.95)
          
          for (i=styr;i<=endyr;i++)
          {
            if (i==yrs_sel_ch_ind(k,isel_ch_tmp)) 
            {
              bu = sel_p1_ind(k,isel_ch_tmp);
              du = sel_p2_ind(k,isel_ch_tmp);
              dd = sel_p3_ind(k,isel_ch_tmp);
			        bd = bu + du + dd;
              if (isel_ch_tmp<n_sel_ch_ind(k))
                isel_ch_tmp++;
            }
...
void model_parameters::Get_Numbers_at_Age(void)
{
  ...
  for (i=styr;i<=endyr;i++)
  {
    if (i<endyr) natage(i+1,1) = SRecruit(Sp_Biom(i)) * mfexp(rec_dev(i+1)) ; //Bai modified
    if (Popes)
    {
    ...
    if (i<endyr) 
		{
      Sp_Biom(i+1)  = elem_prod(natage(i+1),pow(S(i+1),spmo_frac)) * wt_mature; 
...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  if (active(log_Rzero)) // OjO
    ...
    // obj_fun += .5 * square(log_Rzero-R_guess);
  obj_comps.initialize();
...
void model_parameters::Sel_Like(void)
{
  ...
  for (k=1;k<=nind;k++)
  {
    if (active(logsel_p1_ind(k))||active(logsel_slope_ind(k)))
    {
      ...
      sel_like_ind(k,3)    += .1*square( logsel_p2_ind(k,1) )  ;
      ...
      for (i=2;i<=n_sel_ch_ind(k);i++)
      {
        ...
        sel_like_ind(k,3)    += .1*square( logsel_p2_ind(k,i) )  ;
...
void model_parameters::write_mceval_hdr(void)
{
  ...
  random_number_generator& rng= *pad_rng;
    for (k=1;k<=nind;k++)
      mceval<< " model Obj_Fun B100 q_ind_"<< k<< " ";
    mceval<<"M p1 p2 p3 steepness depletion MSY MSYL Fmsy Fcur_Fmsy Bcur_Bmsy Bmsy totbiom_"<<endyr<<" "<< 
    " F35          "<< 
    " F40          "<< 
    " F50          "<< 
...
void model_parameters::Write_R(void)
{
  ...
  for (k=1;k<=nfsh;k++)
  {
    if (nyrs_fsh_length(k)>0)
    {
      ...
      R_report << calc_Francis_weights(olc_fsh(k),elc_fsh(k),n_sample_fsh_length(k),len_bins) <<endl;
  ...
  for (k=1;k<=nind;k++)
  {
    if (nyrs_ind_length(k)>0)
    {
      ...
      R_report << calc_Francis_weights(olc_ind(k),elc_ind(k),n_sample_ind_length(k),len_bins) <<endl;
...
double model_parameters::calc_Francis_weights(const dmatrix olc, const dvar_matrix elc, const ivector sam , const dvector lenbins)
{
  ofstream& mceval= *pad_mceval;
  ofstream& mceval_sr= *pad_mceval_sr;
  ofstream& mceval_R= *pad_mceval_R;
  ...
  random_number_generator& rng= *pad_rng;
  {
    ...
    dvector lengths(olc.colmin(),nlength);
    for (int i=olc.colmin();i<nlength;i++) 
      lengths(i) = (lenbins(i)+lenbins(i+1))/2;
    nobs = olc.rowsize();
    // cout <<nobs<<endl;
    dvector resid(i1,i2);
    resid.initialize();
    for ( int i = i1; i <= i2; i++ )
    {
      // Obs = sum(elem_prod(olc(i), ages+.5));
      Obs = olc(i) * lengths;
      // Pre = sum(elem_prod(value(eac(i)), ages+.5));
      Pre = value(elc(i)) * lengths;
      Var = value(elc(i)) * square(lengths);
      Var -= square(Pre);
...
double model_parameters::calc_Francis_weights(const dmatrix oac, const dvar_matrix eac, const ivector sam )
{
  ...
  random_number_generator& rng= *pad_rng;
  {
    ...
    int i2=oac.rowmax();
    double lfwt,Var,Pre,Obs;
    dvector ages(1,nages);
    for (int i=1;i<=nages;i++) 
      ages(i) = double(i+rec_age - 1)+.5;
    nobs = oac.rowsize();
    // cout <<nobs<<endl;
    dvector resid(i1,i2);
    resid.initialize();
