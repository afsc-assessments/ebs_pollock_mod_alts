...
#' @param fill is color of bars (yellow)
#' @param alpha is transparency of fill
#' @param overlay boolean if you wish them on same panel (default is true)
#' @return Plot of predicted recruitment
#' @author Jim Ianelli
#' @export
#' 
plot_recruitment <- function(M, xlab = "Year", ylab = "Recruitment (millions of individuals)",xlim=c(1990.5,2019.5),fill="yellow",alpha=.9,overlay=TRUE) {
    xlab <- paste0("\n", xlab)
    ylab <- paste0(ylab, "\n")
