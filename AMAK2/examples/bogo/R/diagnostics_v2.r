...
if("input" %in% what){
  ...
  pic <- xyplot(data~year|fleet,data=res,groups=age,
         type="l",lwd=1,
         xlab="Years",ylab="Weight",main="Weight at age in the fishery",
  ...
  if(an(ageFleets)[1] != 0){
    ...
    pic <- barchart(data ~age | fleet* as.factor(year),data=subset(res,year%in% yrs),
            scales=list(rotation=90,alternating=3,y=list(axs="i")),horizontal=F,groups=fleet,strip=F,strip.left=strip.custom(style=1),reverse.rows=T,
            panel=function(...){
              panel.grid(h=-1,v=-1)
              panel.barchart(...)
            },
            main="Age composition in fleets",as.table=T,ylab="Proportion at age")
    print(pic)
    ...
    pic <- levelplot(data ~ age*year | fleet,data=res,
              ...
              zlab="",ylab="",colorkey=T,layout=c(length(ageFleets),1))
    print(pic)
    ...
    for(iFleet in unique(res$fleet)){
      ...
      pic <- xyplot(data ~ age | as.factor(year),data=tmpres,
            ...
            panel=function(x,y){
              ...
              panel.barchart(x,y,horizontal=F,origin=0,box.width=1,col=cols[tmpres$cohort[colidx]])
              })
      print(pic)
    }
  }
  ...
  if(length(which(jjm.in$Inumageyears>0))>0){
    ...
    pic <- barchart(data ~age | survey* as.factor(year),data=subset(res,year%in% yrs),
            scales=list(rotation=90,alternating=3,y=list(axs="i")),horizontal=F,groups=survey,strip=F,strip.left=strip.custom(style=1),reverse.rows=T,
            main="Age composition in surveys",as.table=T,
            panel=function(...){
              panel.grid(h=-1,v=-1)
              panel.barchart(...)
    ...
    pic <- levelplot(data ~ age*year | survey,data=res,
              ...
              zlab="",ylab="",colorkey=T,as.table=F,
    ...
    for(iFleet in unique(tot$fleet)){
      ...
      pic <- xyplot(data ~ length | as.factor(year),data=tmpres,
              ...
              panel=function(x,y){
                yr      <- names(which.max(table(tmpres$year[which(tmpres$data %in% y)])))
                colidx  <- tmpres$cohort[which(tmpres$data %in% y & tmpres$year == an(yr))]
                panel.barchart(x,y,horizontal=F,origin=0,box.width=1,col=tmpres$cohort[colidx],border='transparent')
                })
      print(pic)
...
if("fit" %in% what){
  ...
  pic <- xyplot(resids*scalar ~ year | as.factor(fleet),data=res,
         ...
         cex.axis=1.2,font=2,
         scales=list(y=list(draw=F),alternating=3),
         panel=function(x,y){
          panel.grid(h=-1, v= -1)
  ...
  if(an(ageFleets)[1] != 0){
    ...
    pic <- bubbles(age~year|fleet,data=res,allow.multiple=T,
                   xlab="Years",ylab="Age",main="Residuals catch-at-age by fleet",key=ikey,scales=list(alternating=3))
    print(pic)
  }
  ...
  for(iSurvey in 1:Nsurveys){
    ...
    if(iSurvey == 1) tot <- cbind(resSort,rep(jjm.out$Index_names[iSurvey],nrow(resSort)))
    if(iSurvey != 1){
  ...
  for(iSurvey in 1:Nsurveys){
    ...
    if(iSurvey == 1) tot <- cbind(res,rep(jjm.out$Index_names[iSurvey],nrow(res)))
    if(iSurvey != 1){
      res2  <- cbind(res,rep(jjm.out$Index_names[iSurvey],nrow(res)))
      tot   <- rbind(tot,res2)
    }
  }
  ...
  res$cohort <- res$year - res$age
  pic <- xyplot(data ~ age|sprintf("%i's",floor((year+2)/5)*5) * fleet,res,
  ...
  res$cohort <- res$year - res$age
  pic <- xyplot(data ~ age|sprintf("%i's",floor((year+2)/5)*5) * survey,res,
           groups=year,type="l",as.table=TRUE,
           scale=list(alternating=FALSE),
  ...
  # 19b: Prop F at age
  res <- createDataFrame(t(apply(sweep(jjm.out$TotF,1,rowSums(jjm.out$TotF),"/"),1,cumsum)),jjm.out$N[,1],class=ages)
  ...
  pic <- xyplot(data ~ year,data=res,groups=class,
           ...
           panel=function(...){
            lst <- list(...)
            idx     <- mapply(seq,from=seq(1,length(lst$y),length(lst$y)/length(ages)),to=seq(1,length(lst$y),length(lst$y)/length(ages))+(length(lst$y)/length(ages)-1))
            panel.grid(h=-1,v=-1)
            panel.xyplot(...,col="white")
  ...
  #19c: Prop N at age
  res <- createDataFrame(t(apply(sweep(jjm.out$N[,-1],1,rowSums(jjm.out$N[,-1]),"/"),1,cumsum)),jjm.out$N[,1],class=ages)
  ...
  if(an(lgtFleets)[1] != 0){
    for(iFleet in an(lgtFleets)){
      res <- data.frame(get("jjm.out")[[paste("EffN_Length_Fsh_",iFleet,sep="")]][,c(1,4,5,7,8)])
      colnames(res) <- c("Year","Obs","Model","Obs5","Obs95")
      for(i in 2:5){
        tot <- data.frame(cbind(res[,1],res[,i]))
        tot$class <- names(res)[i]
