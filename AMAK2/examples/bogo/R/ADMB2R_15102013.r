...
read.dat <- function(iFilename,iPath){
                ...
                cols$Flengthcomp          =array (NA,dim=c(nY,nL,nF),dimnames=list(years=Ys[1]:Ys[2],lengths=Ls[1]:Ls[length(Ls)],paste("fishery",1:nF,sep="")))
                for(iFs in 1:nF){
                  if(cols$FnumyearsL[iFs]>0){
                    lFyears       <- rownames(cols$Flengthyears)[which(is.na(cols$Flengthyears[,paste("fishery",iFs,sep="")])==F)]
                ...
                cols$Inumlengthyears[]<- na.omit(an(unlist(res1[counter:(counter+cols$Inum-1)]))); counter <- counter + cols$Inum
                ...
                for(iSu in 1:nI){
                  if(cols$Inumlengthyears[iSu]>0){
                    Iyearslength         <- na.omit(an(res1[[counter]])); wIyearslength <- pmatch(Iyearslength,cols$years[1]:cols$years[2])
                ...
                for(iSu in 1:nI){
                  if(cols$Inumlengthyears[iSu]>0){
                    wIyears       <- rownames(cols$Iyearslength)[which(is.na(cols$Iyearslength[,paste("index",iSu,sep="")])==F)]
                    cols$Ilengthsample[wIyears,iSu] <- na.omit(an(res1[[counter]]))
                    counter       <- counter + 1
                  }
                }
                cols$Iproplength             =array (NA,dim=c(nY,nL,nI),dimnames=list(years=Ys[1]:Ys[2],lengths=Ls[1]:Ls[length(Ls)],paste("index",1:nI,sep="")))
                for(iSu in 1:nI){
                  if(cols$Inumlengthyears[iSu]>0){
                    wIyears       <- rownames(cols$Iyearslength)[which(is.na(cols$Iyearslength[,paste("index",iSu,sep="")])==F)]
                    cols$Iproplength[wIyears,,iSu]   <- matrix(na.omit(an(unlist(res1[counter:(counter+cols$Inumlengthyears[iSu]-1)]))),ncol=nL,
                                                                nrow=cols$Inumlengthyears[iSu],byrow=T)
