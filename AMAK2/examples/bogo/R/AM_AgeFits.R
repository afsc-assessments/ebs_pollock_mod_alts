AgeFits <- function(dat=test,case_label="2011 assessment",f=1) {
  subtle.color <- "gray40"
  ...
  for (yr in 1:nyears) {
    ...
    x <- barplot(obs.data[yr,],space=0.2,ylim=ylim,las=1,names.arg=names.arg, cex.names=0.5, xaxs="i",yaxs="i",border=subtle.color,
                 col=cohort.colors[1:nages,year1-1977],axes=F,ylab="",xlab="")
...
AgeFitsSrv <- function(dat=mod1, main="",case_label="2011 assessment",f=1,rec_age=2) {
  subtle.color <- "gray40"
  attach(dat)
  ages <- c(2,15) #age range
