...
names(sdf)<- c("Year","obs","predicted","post_pred","draw")
sdf2 <- sdf %>% filter(draw==1)
ggplot(sdf2,aes(x=Year,y=obs)) + 
   geom_point(data=sdf,aes(x=jitter(Year),y=post_pred), size=.5,alpha=.4,color="yellow") + 
   geom_point(data=sdf,aes(x=jitter(Year),y=predicted), size=.5,alpha=.1,color="grey") + 
   scale_y_continuous(breaks=seq(0,12e6,by=4e5))  + xlim(c(0,1.6e6)) +
