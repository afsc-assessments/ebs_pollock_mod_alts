...
bdf <- filter(df,yr>1977,yr<=2019)
bdf
bdft <- bdf
for (i in 1:10) bdft <- cbind(bdft,rep(NA,42))
  dim(bdft)
names(bdft)[6:15] <- paste0("SSB_",2018:2009)
i=1
get(paste0("retro",i))$SSB[14:(55-i),1:2]
for (i in 1:10) bdft[1:(42-i),i+5] <- get(paste0("retro",i))$SSB[14:(55-i),2]
bdft
bdf$SSB
for (i in 1:10) bdft[,i+5] <- bdft[,i+5]/bdft[,2]
bdft
p2 <- ggplot(bdft,aes(x=yr,y=SSB)) + scale_x_continuous(breaks=seq(1978,2020,2)) + 
      scale_y_continuous(limits=c(.25,1.75)) + ylab("Relative spawning biomass") + xlab("Year") +  mytheme  
for (i in 1:10) {
  tdf <- data.frame(cbind(bdft[1],SSB=bdft[5+i]))[1:(42-i),]
  names(tdf) <- c("yr", "SSB")
