...
modlyr   <- readList("../archive/2016/mod16.0/For_R.rep")
mod16.0  <- readList("mod16.0/For_R.rep")
mod16.0a <- readList("mod16.0a/For_R.rep")
mod16.0b <- readList("mod16.0b/arc/mod16.0b_R.rep")
mod16.0b <- readList("mod16.0b/For_R.rep")
f.df <- NULL
for (i in 1:dim(mod16.0b$F_age_1)[1] )
  f.df <- rbind(f.df,data.frame(Year=mod16.0b$F_age_1[i,1],F=max(mod16.0b$F_age_1[i,2:12]),Model="2017 assessment"))
for (i in 1:dim(mod16.0b$F_age_1)[1] )
  f.df <- rbind(f.df,data.frame(Year=mod16.0c$F_age_1[i,1],F=max(mod16.0c$F_age_1[i,2:12]),Model="2017 block"))
for (i in 1:dim(mod16.0b$F_age_1)[1] )
  f.df <- rbind(f.df,data.frame(Year=mod16.0d$F_age_1[i,1],F=max(mod16.0d$F_age_1[i,2:12]),Model="Additional block"))

ggplot(f.df,aes(x=Year,y=F,color=Model)) + geom_line(size=2,aes(linetype=Model)) + mytheme + ylab("Apical F")

mod16.0b_n86 <- readList("mod16.0b/arc/mod16.0b_n86_R.rep")
mod16.0b_N50 <- readList("mod16.0b/arc/mod16.0b_N50_R.rep")
mod16.0b_N200 <- readList("mod16.0b/arc/mod16.0b_N200_R.rep")
lstOutN  <- list( "N=50"= mod16.0b_N50,"N=100"= mod16.0b, "N=200"=mod16.0b_N200)
lstOut86 <- list( "2017.Assessment"= mod16.0b, "Drop 1986"= mod16.0b_n86)

mod16.0b <- readList("mod16.0b/For_R.rep")
mod16.0c <- readList("mod16.0c/For_R.rep")
mod16.0d <- readList("mod16.0d/For_R.rep") # one added block
tab       <- cbind(lstOutbl[[1]]$Like_Comp_names,do.call(cbind,lapply(lstOutbl,function(x){round(x[["Like_Comp"]],2)})))
tab
...
cbind(lstOuts[[1]]$Like_Comp_names,
tt <- tibble( srv_sdnr = lstOuts %>% map("sdnr_ind_1"),
  fsha_sdnr = lstOuts %>% map("sdnr_age_fsh_1"),
  srva_sdnr = lstOuts %>% map("sdnr_age_ind_1")
tab       <- cbind(lstOutbl[[1]]$Like_Comp_names,do.call(cbind,lapply(lstOutbl,function(x){round(x[["Like_Comp"]],2)})))

dolstOuts[[1]]$sdnr_ind_1
 t<- do.call(cbind,lapply(lstOuts,function(x){round(x[["sdnr_ind_1"]],2)}))

sdndf <- data.frame(run=names(lstOuts),fsh_age=double()))
for (i in 1:4){
  sdndf$run[i] <- names(lstOuts)[i]
  sdndf$fsh_age[i] <- mean(lstOuts[[i]]$sdnr_age_fsh_1[,2])
}

rbind(cbind(do.call(cbind, lapply(lstOuts,function(x){round(x[["sdnr_ind_1"]],2)}) )),
cbind(do.call(cbind, lapply(lstOuts,function(x){round(x[["Francis_wt_ind_age_1"]],2)}) )) )
cbind(do.call(cbind, lapply(lstOuts,function(x){round(x[["sdnr_age_fsh_1"]],2)}) ))
tt <- data.frame(cbind(do.call(cbind, lapply(lstOuts,function(x){round(x[["sdnr_age_ind_1"]],2)}) )))

names(tt) <- "year"

compareTime(lstOutbl,"SSB",SD=T,Sum=NULL,legendPos="top",startYear=1980)
compareTime(lstOutbl,"F",SD=T,Sum=NULL,legendPos="top",startYear=1980)
compareTime(lstOut86,"SSB",SD=T,Sum=NULL,legendPos="top",startYear=1980)
compareTime(lstOutN,"SSB",SD=T,Sum=NULL,legendPos="top",startYear=1980)
compareTime(lstOutN,"R",SD=T,Sum=NULL,legendPos="top",startYear=1980)

compareTime(lstOutb,"SSB",SD=T,Sum=NULL,legendPos="top",startYear=1980)
compareTime(lstOutb,"R",SD=T,Sum=NULL,legendPos="top",startYear=1980)
compareTime(lstOutbc,"SSB",SD=T,Sum=NULL,legendPos="top",startYear=1980)
...
compareTime(lstOutbc,"R",SD=T,Sum=NULL,legendPos="top",startYear=1980)

rdf <- rbind(data.table(mod16.0b_N50$R,"N=50"), data.table(mod16.0b$R,"N=100"), data.table(mod16.0b_N200$R,"N=200"))
rdf <- rbind(data.table(mod16.0b$R,"Model_16.0b"), data.table(mod16.0b_n86$R,"Without 1986 survey"))
rdf <- rbind(data.table(modlyr$R,"2016 Assessment"), data.table(mod16.0a$R,"Current assessment"))
names(rdf) <- c("yr","R","se","lb","ub","case")
rdf$case <- as.factor(rdf$case)
rdf$case <- factor(rdf$case,levels(rdf$case)[c(3,1,2)])
mytheme = mytheme + theme(axis.text.x = element_text(angle=90, hjust=1, vjust=.5))
rdf  <- rdf[yr>1990&yr<2018,.(yr,R=R,lb=lb,ub=ub,case)]
rdf  <- rdf[yr>1978&yr<2018,.(yr,R=R,lb=lb,ub=ub,case)]
mnR <- mean(mod16.0b$R[,2])
mnR_n86 <- mean(mod16.0b_n86$R[,2])
1-mnR_n86/mnR

mnB <- mean(mod16.0b$SSB[54,2])
dim(mod16.0b$SSB)
mnB_n86 <- mean(mod16.0b_n86$SSB[54,2])
1-mnB_n86/mnB
...
ggplot(rdf,aes(x=yr-1,y=R,fill=case)) + xlab("Year class") + ylab("Age 1 recruits (thousands)") + #ylim(c(0,18000)) +
       ...
       scale_x_continuous(breaks=seq(1978,2017,2)) +
       geom_errorbar(aes(ymin=lb,ymax=ub),width=.3,colour="blue",position=dodge) + mytheme + 
       geom_hline(aes(yintercept=mnR_n86)) + geom_hline(aes(yintercept=mnR))

likes_86 <- data.frame( rbind( mod16.0b$Like_Comp, mod16.0b_n86$Like_Comp))
  names(likes_86) <- mod16.0b$Like_Comp_names
  mod16.0b$q_1
  mod16.0b_n86$q_1
  1 - mod16.0b_n86$q_1[1] / mod16.0b$q_1[1] 

#-------------------------------------------------------------------------------
# Fit to survey data
caplyr <- "2016 assessment"
captyr <- "Current assessment "
IndexFit(modlyr,yf=1990,yl=2017,f=1,main=caplyr,ylab="Survey biomass (t)")
IndexFit(mod16.0b,yf=1990,yl=2017,f=1,main=captyr,ylab="Survey biomass (t)")
IndexFit(mod16.0,yf=1990,yl=2017,f=1,main=captyr,ylab="Survey biomass (t)")
AgeFitsSrv(mod16.0c,rec_age=1,case_label=caplyr)
AgeFitsSrv(mod16.0d,rec_age=1,case_label=captyr)
...
AgeFits(mod16.0d,rec_age=1,case_label="16.0d")
AgeFits(mod16.0b,rec_age=1,case_label="16.0b")

dt <- data.table(modlyr$Obs_Survey_1[,1:4],mod16.0$Obs_Survey_1[,3])
names(dt) <- c("Year","Observed","Model_17.0","sd","Model_16.0")
dt <- data.table(dt)[Year>1986,.(Year, Observed, Model_17.0,Model_16.0, lb=Observed-1.96*sd,ub=Observed+1.96*sd),]
dtp <-melt(dt,id="Year")
dtp
ggplot(dt,aes(x=Year,y=Model_16.0) ) + geom_point(data=dtp[variable=="Observed"],size=3) + geom_line(data=dtp[variable!="Observed"]) + ylim(c(0,1800000)) + labs(x="Year",y="Survey biomass") + mytheme #+
...
df <- data.frame(mod16.0b_N50$sel_fsh_1[,2:13] ); names(df) <- c("yr",1:11)
df <- data.frame(mod16.0b_N200$sel_fsh_1[,2:13] ); names(df) <- c("yr",1:11)
sdf <- gather(df,age,sel,2:12) %>% filter(yr>1976) %>% mutate(age=as.numeric(age)) #+ arrange(age,yr)
data.frame()
dtmp <- data.frame(Model="N=100",age=1:11,sel=colMeans(mod16.0b$sel_fsh_1[36:40,3:13]) / max(colMeans(mod16.0b$sel_fsh_1[36:40,3:13]) ))
dtmp <- rbind(dtmp,data.frame(Model="N=50",age=1:11,sel=colMeans(mod16.0b_N50$sel_fsh_1[36:40,3:13]) /max(colMeans(mod16.0b_N50$sel_fsh_1[36:40,3:13]) )))
dtmp <- rbind(dtmp,data.frame(Model="N=200",age=1:11,sel=colMeans(mod16.0b_N200$sel_fsh_1[36:40,3:13]) /max(colMeans(mod16.0b_N200$sel_fsh_1[36:40,3:13]) )))
ggplot(dtmp, aes(x=age,y=sel,color=Model)) + mytheme + geom_line(size=2)
...
df <- data.frame(mod16.0a$sel_fsh_1[,2:13] ); names(df) <- c("yr",1:11)
df <- data.frame(mod16.0b$sel_fsh_1[,2:13] ); names(df) <- c("yr",1:11)
sdf <- gather(df,age,sel,2:12) %>% filter(yr>1976) %>% mutate(age=as.numeric(age)) #+ arrange(age,yr)
head(sdf)
...
bdf <- filter(df,yr>1980,yr<=2017) %>% arrange(yr)
bdf
...
p1 <- ggplot(data=bdf,aes(x=yr,y=SSB,alpha=0.3,color=Model)) + scale_y_continuous(limits=c(0,600000)) + ylab("Spawning biomass") + 
          xlab("Year") +  mytheme + geom_line(data=bdf,aes(x=yr,y=SSB,type=Model)) +
...
rdf <- rbind(data.table(mod16.0b$R,"N=100"), 
  data.table(mod16.0b_N50$R,"N=50"),
  data.table(mod16.0b_N200$R,"N=200"),
  data.table(mod16.0b_n86$R,"Drop 1986"))
names(rdf) <- c("yr","R","se","lb","ub","case")
mytheme = mytheme + theme(axis.text.x = element_text(angle=90, hjust=1, vjust=.5))
#rdf  <- rdf[yr>1990&yr<2017,.(yr,R=R/mean(R),lb=lb/mean(R),ub=ub/mean(R),case)]
mnR <- rdf %>% filter(yr<2017,yr>1977) %>% group_by(case)%>% summarise(mean=mean(R) )
mnR
rdf  <- rdf[yr>1978&yr<2018,.(yr,R=R,lb=lb,ub=ub,case)]
dodge <- position_dodge(width=0.8)
ggplot(rdf,aes(x=yr-1,y=R,fill=case)) + xlab("Year class") + ylab("Age 1 recruits (thousands)") + #ylim(c(0,18000)) +
...
for (i in 0:15) {
#sdf <- gather(df,age,sel,2:12) %>% filter(yr>1976) %>% mutate(age=as.numeric(age)) #+ arrange(age,yr)
  dtmp <- read.table(paste0("mod16.0b/retro/r_NLL_",i,".rep"))
  ...
  dtmp <- data.frame(get(mn)$Obs_Survey_1) %>% mutate_all( function(x) as.numeric(as.character(x)))
  df   <- data.frame(get(mn)$sel_fsh_1[,2:13] ); names(df) <- c("yr",1:11)
  sdf <- gather(df,age,sel,2:12) %>% filter(yr>1976) %>% mutate(age=as.numeric(age)) #+ arrange(age,yr)
  sdf$peel <- i
  sel <- rbind(sel,sdf)
  names(dtmp) <- c("Year","Observed","Predicted","Std_dev","Res1","Res2")
  dtmp$peel <- i
  srv <- rbind(srv,dtmp)
  print(rn)
}
srv$Terminal_year <- as.factor(2017-srv$peel) 
srv <- srv %>% filter(Year>1986)
ggplot(srv,aes(x=Year,y=Predicted,color=Terminal_year)) + geom_line() + geom_point(aes(x=Year,y=Observed),size=5) + 
mytheme + xlim(c(1990,2018)) + ylim(c(0,1.2e6)) + theme(legend.position="none") + ylab("Survey index")

nll$Terminal_year <- as.factor(2017-nll$peel) 
...
ggplot(nll,aes(x=Year,y=Fishery_age,color=Terminal_year)) + geom_line() + geom_point() + mytheme+ theme(legend.position="none") + ylab("Fishery age -log likelihood")
nll %>%filter(Year>2012)
#df <- data.frame(modlyr$sel_fsh_1[,2:13] ); names(df) <- c("yr",1:11)
#sdf <- gather(df,age,sel,2:12) %>% filter(yr>1976) %>% mutate(age=as.numeric(age)) #+ arrange(age,yr)
ggplot(sel,aes(x=age,y=as.factor(yr),height = sel)) + geom_density_ridges(stat = "identity",scale = 5.8, alpha = .2) + xlim(c(1,11))+ mytheme + ylab("Year") + xlab("Age (years)") #+ theme_ridges()  
sel %>% arrange(yr,age) %>%mutate(peel=as.factor(peel),Year=yr+sel)%>% ggplot(aes(x=age,y = Year,group=yr,color=peel)) + geom_line() + xlim(c(1,11))+ mytheme + ylab("Year") + xlab("Age (years)") #+ theme_ridges()  
retouts <- list()
...
for (i in 1:10) {
print(c(i,retouts[[i]]$Q_Survey_1))
}
...
p1 <- ggplot() + scale_y_continuous(limits=c(0,530000)) + ylab("Spawning biomass") + xlab("Year") +  mytheme + geom_line(data=bdf,aes(x=yr,y=SSB),size=4) +
           geom_ribbon(data=bdf ,aes(x=yr,y=SSB,ymin=lb,ymax=ub),fill="lemonchiffon",col="grey",alpha=.6)  + guides(fill=FALSE,alpha=FALSE,col=FALSE) + scale_x_continuous(breaks=seq(1978,2018,2)) 
for (i in 1:10) {
...
for (i in 1:5){
  df <- as.data.frame( lstOuts[[i]]$Obs_Survey_1[-1,] )
