...
lstOut1   <- list( "2016.Assessment"= modlyr, "Current.assessment"= mod16.0,"Model.16.0b"=mod16.0b)
lstOuts   <- list( "Lastyr"=modlyr,
                   "Model_16.0"= mod16.0,
                   "Model_16.0a"= mod16.0a,
                   "Model_16.0b"= mod16.0b,
                   "Model_16.0c"= mod16.0c )
lstOutb   <- list( "Model_16.0b"= mod16.0b,
                   "Model_16.0"= mod16.0)
lstOutbc  <- list( "Model_16.0b"= mod16.0b,
                   "Model_16.0c"= mod16.0c)
...
map(lstOuts,)
map2(lstOuts,sumbiom)
map_df(data.table(lstOuts),"[","sumbiom")
names(lstOuts[[1]])
...
rdf <- rbind(data.table(mod16.0b$R,"Model_16.0b"), data.table(mod16.0c$R,"Model_16.0c "))
...
AgeFitsSrv(mod16.0,rec_age=1,case_label=captyr)
...
sdf <- gather(df,age,sel,2:12) %>% filter(yr>1976) %>% mutate(age=as.numeric(age)) #+ arrange(age,yr)
head(df)
ggplot(sdf,aes(x=age,y=as.factor(yr),height = sel)) + geom_density_ridges(stat = "identity",scale = 5.8, alpha = .9,color="blue",fill="yellow",size=.5)+ xlim(c(1,11))+ mytheme + ylab("Year") + xlab("Age (years)") #+ theme_ridges()  
ggplot(sdf,aes(x=age,y=as.factor(yr),height = sel)) + geom_density_ridges(stat = "identity",scale = 5.8, alpha = .4,color="black",fill="orange",size=.5)+ xlim(c(1,11))+ mytheme + ylab("Year") + xlab("Age (years)") #+ theme_ridges()  
ggplot(sdf,aes(x=age,y=as.factor(yr),height = sel)) + geom_density_ridges(stat = "identity",scale = 5.8, alpha = .2,fill="orange")+ xlim(c(1,11))+ mytheme + ylab("Year") + xlab("Age (years)") #+ theme_ridges()  
