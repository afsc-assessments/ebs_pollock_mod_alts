...
m <- d <- 'amak' # the model name, folder is also assumed to be called this
## First optimize the model to make sure the Hessian is good.
setwd(m); system('pm -nox -iprint 200 -mcmc 15 -ainp pm.par -phase 50'); setwd('..')
...
setwd(m); system(paste(m, '-hbf 1 -nox -mcmc 15 -binp pm.bar -phase 22')); setwd('..')
