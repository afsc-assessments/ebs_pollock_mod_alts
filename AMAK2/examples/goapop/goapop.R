...
#-------------------------------------------------------------------------------
#Selectivity
pdir <- "/Users/jim/_mymods/afsc-assessments/rockfish/examples/GOAPOP"
p0<- readList(paste0(pdir,'/pop_R.rep'))
m0<- readList('model_0/For_R.rep')
m1<- readList('model_1/For_R.rep')
m2<- readList('model_2/For_R.rep')
m3<- readList('model_3/For_R.rep')
df <- data.frame(p0$sel_fsh_1[,2:23] ); names(df) <- c("yr",1:21)
df <- data.frame(m0$sel_fsh_1[,2:23] ); names(df) <- c("yr",1:21)
df <- data.frame(m1$sel_fsh_1[,2:23] ); names(df) <- c("yr",1:21)
df <- data.frame(m2$sel_fsh_1[,2:23] ); names(df) <- c("yr",1:21)
df <- data.frame(m3$sel_fsh_1[,2:23] ); names(df) <- c("yr",1:21)
sdf <- gather(df,age,sel,2:22) %>% filter(yr>1970) %>% mutate(age=as.numeric(age)) #+ arrange(age,yr)
ggplot(sdf,aes(x=age,y=as.factor(yr),height = sel)) + geom_density_ridges(stat = "identity",scale = 5.8, alpha = .2,color="blue",fill="yellow",size=.5)+ xlim(c(1,21))+ ylab("Year") + xlab("Age (years)") + scale_y_discrete(limits=rev(levels(as.factor(sdf$yr)))) + theme_few()
psel <- function(M,title="") {
  df  <- data.frame(M$sel_fsh_1[,2:23] ); names(df) <- c("yr",1:21)
  sdf <- gather(df,age,sel,2:22) %>% filter(yr>1970) %>% mutate(age=as.numeric(age)) #+ arrange(age,yr)
...
#Rec ============================================================
rdf <- cbind(data.frame(m2$R,case="3pDL, blocks"))
rdf <- rbind(rdf,cbind(data.frame(p0$R,case="Hulson, blocks")))
